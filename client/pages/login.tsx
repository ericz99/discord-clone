import React from "react";
import { useRouter } from "next/router";
import Link from "next/link";

import { InputField, Button } from "../components";
import { useFormField, useErrors } from "../shared/hooks";

import { useLoginUserMutation } from "../shared/generated/schema";

import {
  authToken,
  ensureSignIn,
  getAccessToken,
  setAccessToken,
} from "../shared/auth";

export default function LoginPage() {
  const router = useRouter();
  const { errors, setErrors, formatErrors } = useErrors();
  const { handleChange, inputs } = useFormField();
  const [loginUser, { loading }] = useLoginUserMutation({
    errorPolicy: "all",
  });

  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const { email, password } = inputs;

    // # execute mutation
    const { data, errors } = await loginUser({
      variables: {
        email,
        password,
      },
    });

    if (!loading && errors) {
      formatErrors(errors);
    } else {
      // # clear errors
      setErrors({});
      // # set token to localstorage
      // localStorage.setItem("token", data.loginUser.access_token);
      setAccessToken(data.loginUser.access_token);
      console.log(getAccessToken());
      authToken(data.loginUser.access_token);

      // # just go to url
      // window.location.href = "/channels";
      router.push("/channels");
    }
  };

  return (
    <div className="bg-slate-900 h-full flex items-center justify-center">
      <div className="container mx-auto flex items-center justify-center">
        <div className="min-w-full lg:min-w-[33%] md:min-w-[75%] lg:mx-12">
          <form className="flex flex-col px-8" onSubmit={handleSubmitForm}>
            <InputField
              label="Email"
              type="email"
              name="email"
              onChange={(e) => handleChange(e)}
              value={inputs.email ?? ""}
              hasError={errors.email || ""}
              placeholder="test@yahoo.com"
              required
            />
            <InputField
              label="Password"
              type="password"
              name="password"
              onChange={(e) => handleChange(e)}
              value={inputs.password ?? ""}
              hasError={errors.password || ""}
              required
            />

            <div className="mb-5 flex justify-end">
              <Link href="/register" className="font-xs text-white font-bold">
                Don&apos;t have an account?
              </Link>
            </div>

            <Button
              type="submit"
              className="bg-lime-600 w-full font-xs p-2.5 text-white mx-auto rounded-md font-bold"
            >
              Login Now
            </Button>
          </form>
        </div>
      </div>
    </div>
  );
}

export async function getServerSideProps({ req }) {
  return ensureSignIn(req);
}
